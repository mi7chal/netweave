use crate::integrations::{
    IntegrationDhcpLease, IntegrationNetwork, IntegrationProvider, IntegrationType,
};
...
impl AdGuardIntegration {
    ...
    async fn login(&self) -> Result<()> {
        ...
        let res = self.client.post(&login_url).json(&payload).send().await?;
    ...
    async fn ensure_authenticated(&self) -> Result<()> {
        ...
        if !res.status().is_success() || res.status() == 403 || res.status() == 401 {
            tracing::debug!(
                "AdGuard session invalid (Status={}), re-authenticating...",
                res.status()
            );
            self.login().await?;
        }
...
impl IntegrationProvider for AdGuardIntegration {
    ...
    async fn fetch_networks(&self) -> Result<Vec<IntegrationNetwork>> {
        ...
        if !res.status().is_success() {
            tracing::warn!(
                "AdGuard fetch_networks: DHCP might be disabled (Status={})",
                res.status()
            );
            return Ok(Vec::new());
        }
        ...
        tracing::info!(
            "AdGuard DHCP status response keys: {:?}",
        ...
        tracing::info!(
            "AdGuard discovered stats: gateway={:?}, mask={:?}",
            gateway,
            mask
        );
        ...
        if let (Some(gateway), Some(mask)) = (gateway, mask) {
            if !gateway.is_empty() && !mask.is_empty() {
                ...
                if let (Ok(gw_ip), Ok(m_ip)) = (
                    ...
                    tracing::info!(
                        "Discovered network from AdGuard: {} with CIDR {}",
                        gateway,
                        cidr
                    );
    ...
    async fn fetch_devices(&self) -> Result<Vec<IntegrationDhcpLease>> {
        ...
        if !res.status().is_success() {
            tracing::warn!(
                "AdGuard fetch_devices: DHCP might be disabled (Status={})",
                res.status()
            );
            return Ok(Vec::new());
        }
        ...
        let json: Value = match serde_json::from_str(&text) {
            ...
            Err(e) => {
                tracing::warn!(
                    "AdGuard fetch_devices: Failed to parse DHCP status JSON: {} (Body: {})",
                    e,
                    text
                );
                return Ok(Vec::new());
    ...
    async fn push_static_lease(&self, mac: &str, ip: &str, hostname: &str) -> Result<()> {
        ...
        if !res.status().is_success() {
            ...
            let body = res.text().await.unwrap_or_default();
            tracing::error!(
                "AdGuard push_static_lease Failed: Status={}, Body={}",
                status,
                body
            );
            ...
            return Err(anyhow::anyhow!(
                "AdGuard push_static_lease Failed: {} - {}",
                status,
                body
            ));
        }
        ...
        tracing::info!(
            "Successfully pushed static lease to AdGuard: {} -> {}",
            mac,
            ip
        );
        Ok(())
    ...
    async fn delete_static_lease(&self, mac: &str, ip: &str, hostname: &str) -> Result<()> {
        ...
        if !res.status().is_success() {
            ...
            let body = res.text().await.unwrap_or_default();
            tracing::error!(
                "AdGuard delete_static_lease Failed: Status={}, Body={}",
                status,
                body
            );
            ...
            if status == 400 {
                tracing::info!(
                    "Static lease might not exist in AdGuard to delete: {} - {}",
                    mac,
                    ip
                );
                return Ok(());
            ...
            return Err(anyhow::anyhow!(
                "AdGuard delete_static_lease Failed: {} - {}",
                status,
                body
            ));
        }
        ...
        tracing::info!(
            "Successfully deleted static lease from AdGuard: {} -> {}",
            mac,
            ip
        );
        Ok(())
