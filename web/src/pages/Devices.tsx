...
export const Devices = () => {
    ...
    const { data: devices, error, isLoading, mutate, remove } = useCRUDList<DeviceListView>({
        endpoint: "/api/devices",
        onError: (e) => toast.error("Failed to load devices", { description: e.message }),
    });

    const { searchTerm: search, setSearchTerm: setSearch, filteredData } = useTableSearch(devices, {
        searchableFields: ["hostname", "device_type", "primary_ip", "mac_address", "os_info"],
    });

    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [formData, setFormData] = useState<Partial<CreateDevicePayload>>(defaultDeviceFormData());
        ...
        <AppLayout>
            <div className="flex flex-col space-y-6">
                <PageHeader title="Devices" description="Manage infrastructure hardware and virtual machines.">
                    ...
                    <Button onClick={() => { setFormData(defaultDeviceFormData()); setIsDialogOpen(true); }} className="gap-2 shadow-sm rounded-full h-10 px-5 flex-shrink-0 hover:scale-105 transition-all duration-300">
                ...
                >
                    <DialogContent className="sm:max-w-[425px] bg-card/80 backdrop-blur-2xl border-border/40 shadow-2xl">
                        ...
                        <FormWrapper onSubmit={async () => { await handleSave(); }}>
                            <div className="grid gap-4 py-6">
                                ...
                                <div className="grid gap-2">
                                    ...
                                    >
                                        <SelectTrigger className={deviceSelectTriggerClass}><SelectValue placeholder="Select type" /></SelectTrigger>
                            ...
                            <DialogFooter className="border-t border-border/20 pt-4 mt-2">
                                <Button type="button" variant="outline" onClick={() => setIsDialogOpen(false)} className="hover:bg-secondary/60">Cancel</Button>
